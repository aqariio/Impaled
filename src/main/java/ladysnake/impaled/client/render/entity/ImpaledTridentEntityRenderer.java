...
public class ImpaledTridentEntityRenderer extends EntityRenderer<ImpaledTridentEntity> {
    ...
    public void render(ImpaledTridentEntity trident, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        matrices.push();
        matrices.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(MathHelper.lerp(tickDelta, trident.prevYaw, trident.getYaw()) - 90.0F));
        matrices.multiply(Vec3f.POSITIVE_Z.getDegreesQuaternion(MathHelper.lerp(tickDelta, trident.prevPitch, trident.getPitch()) + 90.0F));
